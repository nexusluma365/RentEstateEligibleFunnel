...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (pi.status === 'succeeded') {
      ...
      const entitlements = await patchEntitlements(leadId, patch);
      ...
      return { statusCode: 200, body: JSON.stringify({ ok: true, status: 'succeeded', entitlements }) };
    }
    ...
    return { statusCode: 500, body: JSON.stringify({ ok: false, error: 'Could not confirm payment.' }) };
  }
};
